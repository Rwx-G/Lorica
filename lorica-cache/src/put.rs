...
// maybe this can simplify some logic in lorica::h1
...
mod parse_response {
    ...
    mod test {
        ...
        fn test_partial_response_headers() {
            ...
            assert_eq!(header.status, 200);
            assert_eq!(header.headers.get("Server").unwrap(), "lorica");
            assert!(!eos);
        }
