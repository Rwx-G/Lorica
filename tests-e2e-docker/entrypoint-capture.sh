#!/bin/sh
# =============================================================================
# Lorica capture E2E entrypoint (Stories 10.1 / 10.2). Identical to the
# log-sinks entrypoint (the boot log is copied to `/shared/lorica.log`
# so the runner can assert the `lorica::capture` stdout sink) plus one
# thing: a writable capture directory under the shared volume, created
# here because the directory writer never creates it (docs/capture.md,
# "Sinks"). The runner mounts `/shared` read-only and reads the files
# as root. The read-only `output.dir` the smoke also needs is a named
# volume mounted `:ro` by compose, not this script.
# =============================================================================
mkdir -p /shared
