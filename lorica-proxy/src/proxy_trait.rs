...
pub trait ProxyHttp {
    ...
    ///
    /// See `lorica-proxy/tests/utils/server_utils.rs` for a minimal (not
    ...
    fn cache_not_modified_filter(
        ...
        Ok(
            lorica_core::protocols::http::conditional_filter::not_modified_filter(
                session.req_header(),
                resp,
            ),
        )
    ...
    /// leaves neither `Content-Length` nor `Transfer-Encoding`, Lorica adds
