...
//! # lorica-proxy
//!
//! Programmable HTTP proxy built on top of [lorica_core].
//!
//! # Features
...
//!
//! Then the service can be passed into [`http_proxy_service()`] for a [lorica_core::server::Server] to
//! run it.
//!
...
use lorica_cache::NoCacheReason;
use lorica_core::apps::{
    HttpPersistentSettings, HttpServerApp, HttpServerOptions, ReusedHttpStream,
};
use lorica_core::connectors::http::custom;
use lorica_core::connectors::{http::Connector, ConnectorOptions};
use lorica_core::modules::http::compression::ResponseCompressionBuilder;
use lorica_core::modules::http::{HttpModuleCtx, HttpModules};
use lorica_core::protocols::http::client::HttpSession as ClientSession;
use lorica_core::protocols::http::custom::CustomMessageWrite;
use lorica_core::protocols::http::subrequest::server::SubrequestHandle;
use lorica_core::protocols::http::v1::client::HttpSession as HttpSessionV1;
use lorica_core::protocols::http::v2::server::H2Options;
use lorica_core::protocols::http::HttpTask;
use lorica_core::protocols::http::ServerSession as HttpSession;
use lorica_core::protocols::http::SERVER_NAME;
use lorica_core::protocols::Stream;
use lorica_core::protocols::{Digest, UniqueID};
use lorica_core::server::configuration::ServerConf;
use lorica_core::server::ShutdownWatch;
use lorica_core::upstreams::peer::{HttpPeer, Peer};
use lorica_error::{Error, ErrorSource, ErrorType::*, OrErr, Result};
...
use lorica_cache::HttpCache;
use lorica_core::protocols::http::compression::ResponseCompressionCtx;
...
error[E0391]: cycle detected when computing type of `proxy_cache::<impl at lorica-proxy/src/proxy_cache.rs:7:1: 7:23>::proxy_cache::{opaque#0}`
   --> lorica-proxy/src/proxy_cache.rs:13:10
    |
...
use lorica_core::services::listening::Service;
...
///
/// The returned [Service] can be hosted by a [lorica_core::server::Server] directly.
pub fn http_proxy_service<SV>(conf: &Arc<ServerConf>, inner: SV) -> Service<HttpProxy<SV, ()>>
where
    SV: ProxyHttp,
{
    http_proxy_service_with_name(conf, inner, "Lorica HTTP Proxy Service")
}
    ...
    SV::CTX: Send + Sync + 'static,
{
    ...
    pub fn new(conf: &Arc<ServerConf>, inner: SV) -> Self {
        ProxyServiceBuilder {
            conf: conf.clone(),
            inner,
            name: "Lorica HTTP Proxy Service".into(),
            connector: (),
            custom: None,
            server_options: None,
        }
    ...
    C: custom::Connector,
{
    ...
    ///
    /// The returned [Service] is ready to be used by a [lorica_core::server::Server].
