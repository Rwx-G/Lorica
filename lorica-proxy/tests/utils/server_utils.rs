...
use lorica_cache::lock::CacheKeyLockImpl;
use lorica_cache::{
...
use lorica_core::server::configuration::Opt;
use lorica_core::services::{Service, ServiceWithDependents};
use lorica_core::upstreams::peer::HttpPeer;
use lorica_core::utils::tls::CertKey;
...
impl ProxyHttp for ExampleProxyCache {
    ...
    fn request_cache_filter(&self, session: &mut Session, _ctx: &mut Self::CTX) -> Result<()> {
        ...
        let eviction = session.req_header().headers.get("x-eviction").map(|_| {
            &*EVICTION_MANAGER as &'static (dyn lorica_cache::eviction::EvictionManager + Sync)
        });
        let lock = session
            .req_header()
            .headers
...
fn test_main() {
    ...
    #[cfg(feature = "any_tls")]
    {
        let mut proxy_service_https =
            lorica_proxy::http_proxy_service(&my_server.configuration, ExampleProxyHttps {});
        proxy_service_https.add_tcp("0.0.0.0:6149");
        let cert_path = format!("{}/tests/keys/server.crt", env!("CARGO_MANIFEST_DIR"));
        let key_path = format!("{}/tests/keys/key.pem", env!("CARGO_MANIFEST_DIR"));
        let mut tls_settings =
            lorica_core::listeners::tls::TlsSettings::intermediate(&cert_path, &key_path).unwrap();
    ...
    let mut proxy_service_cache =
        lorica_proxy::http_proxy_service(&my_server.configuration, ExampleProxyCache {});
...
impl PskTlsServer {
    ...
    async fn run_server() {
        use lorica_core::{protocols::tls::S2NConnectionBuilder, tls::TlsAcceptor};
        use lorica_core::{
