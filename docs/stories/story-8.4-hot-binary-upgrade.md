...
- dist/lorica.service (Type=notify), dist/build-deb.sh, dist/rpm/lorica.spec
- docs/installation.md (new), docs/hot-upgrade.md (new), docs/security.md
- tests-e2e-docker/: entrypoint-hot-upgrade.sh (new), test-runner/run-hot-upgrade-smoke.sh (new), docker-compose.yml, Dockerfile (+ lorica-metrics COPY), test-runner/Dockerfile, run.sh
