...
|------|---------|-------------|--------|
| 2026-09-09 | 1.1 | Epic close: this story's Integration Verification ran for real. The `cluster` profile's first version asserted only the SHAPE of entitlement (no certificate was ever bound, so its "nothing to distribute" branch fired unconditionally while the header claimed the key guarantee was tested; quality audit, High). The profile now includes the Pebble fixture the PRD named as its prerequisite: an HTTP-01 order for `fleet.example.com` whose validation Pebble completes through edge-a only (the A record points at the selected follower, so the token had to fan out), the issued certificate bound to the selected route, the key install logged on edge-a and absent on edge-b, and the roster's entitlement column agreeing. `selected_hostnames` in the roster endpoint now really is one store pass (performance audit, High: it walked the route table once per node name under the store lock while its comment said "one pass"). | Romain G. |
