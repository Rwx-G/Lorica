...
|------|---------|-------------|--------|
| 2026-09-09 | 1.2 | Epic close (security audit, High): revocation cut a node's access and nothing said what it kept. `revoke_node` now reads the node's certificate entitlement BEFORE flipping the row and returns it as `certificates_to_reissue` (200 with a body instead of 204), in the audit row and on the `ClusterNodeLeft` alert; `docs/cluster.md`, the hardening guide and the threat model say that access is cut and key material is not recovered. The follower gate admits `leave`, `break-glass` and `status` by exact path instead of the `/api/v1/cluster/` prefix (test extended with the five fleet mutations). The e2e profile now revokes both followers and asserts the list. | Romain G. |
| 2026-09-09 | 1.1 | Defect found by the first run of the `cluster` e2e profile (backlog #66), fixed in the same pass. `LiveSession.state` was captured at admission and never updated: a node admitted `Pending` and activated through the API while its session was open stayed `Pending` in the registry, so the configuration pull, the certificate pull, the telemetry push and the replication coordinator's target list all kept refusing it until it happened to reconnect, while the roster row and the dashboard both said active. AC #5's gate worked; activation did not, for any node already connected. Every roster refresh now reconciles live sessions, and the three listener gates read the live entry rather than the identity captured at admission. Pinned by `a_roster_refresh_flips_a_live_session_s_state` and by the e2e assertion that both followers converge after activation. This story's Integration Verification ran for the first time in that same profile run. | Romain G. |
