...
|------|---------|-------------|--------|
| 2026-09-09 | 1.2 | Epic close. Each telemetry class advances its own cursor on its own accepted count (architecture review, High): gated on one conjunction, a class an older control plane did not acknowledge would have frozen access, WAF and audit together, forever, silently. `TelemetryPush` gained the per-session rate cap the other expensive methods already had (240 per minute, answered with a back-off acknowledgement); `IngestQuota` sweeps entries whose window rolled over, so revocation and re-enrolment no longer leave one spend entry per historical node; the hourly pass reclaims the fan-in rows of node ids absent from the roster and of nodes revoked more than a day ago. `docs/cluster.md` gained the four-class taxonomy (the heartbeat rule now says "bounded in size", the clause bans needed) and states the one silent loss (partition beyond local retention, backlog #76). | Romain G. |
| 2026-09-09 | 1.1 | Two items this record carried forward are closed at the epic close, as D10 and D13 said they would be. D10: `metrics_require_auth` now defaults to `true` (one default function serves both the serde attribute and `Default`, so a TOML import and a fresh store cannot disagree), with the migration paragraph in the CHANGELOG and `docs/security.md`, and the base e2e suite's old "accessible without auth" assertion turned into the flip's own regression test. Backlog #66: the `cluster` e2e profile exists and this story's Integration Verification has run against a real control plane and two followers, one in workers mode: access rows and WAF events reached the control plane stamped with the node the session proved. The ceiling stays derived rather than measured (backlog #64). Status Done. | Romain G. |
