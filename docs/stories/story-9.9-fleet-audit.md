...
|------|---------|-------------|--------|
| 2026-09-09 | 1.3 | Epic close (security audit, High): two of backlog #72's stated mitigations did not exist. The decode boundary never checked the timestamp's shape, so a far-future stamp held a row forever; fanned-in rows now carry `received_at`, set by this store, and retention ages every chain by arrival (local rows by their own stamp, the same clock), with the prefix-cut invariant unchanged and pinned by `a_far_future_timestamp_does_not_keep_a_fanned_in_row_forever`. The rate was unbounded; the per-session push cap is in Story 9.6's record. #72 corrected; the shared-connection contention raised as #75 to measure. | Romain G. |
