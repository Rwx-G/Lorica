...
5. **Scopes** are a closed enum: `environments:write` (create, update, delete environments), `environments:read`, `routes:read`, `certificates:read`. There is deliberately no `routes:write`, no `certificates:write`, and no `settings:*` in 1.8.0: the automation surface is the environment resource, not the whole management API behind a different door. **Revisited in v1.9.0**: [Epic 11](./epic-11-v1.9.0.md) extends this enum with read scopes in Story 11.1 and reverses the write exclusion in Story 11.2, for a different caller and with its own justification. The exclusion stands for 1.8.0 and for the CI caller this story serves.
