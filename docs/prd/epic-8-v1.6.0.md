...
1. `lorica/src/main.rs` shrinks to ~150 LOC of dispatch. New layout: `cli/{mod, serve, rotate_key, reset_admin, unban}.rs` + `startup/{mod, encryption_key, signals, workers, runtime, background_tasks}.rs`.
2. `BackgroundTasks::start(&store, &task_tracker, &alert_sender, role: Role) -> BackgroundTaskHandles` owns the ~200 LOC of common spawns (notify dispatcher, persisted-alert dispatcher, SLA collector flush, probe scheduler reload, load-test engine, ACME challenge store, ACME renewal task, cert-expiry check, basic-auth-cache prune, rate-limit prune, bot-stash prune, GeoIP / ASN handles, rDNS resolver, blocklist refresh, OTel try-init).
3. `run_supervisor` / `run_worker` / `run_single_process` each shrink to ~150 LOC of role-specific wiring + one `BackgroundTasks::start(role)` call.
4. `lorica/src/proxy_wiring.rs` is split into `proxy_wiring/{mod, routing, backends, filters/{cache, waf, rate_limit, forward_auth, bot}, context, response, helpers}.rs`.
5. `request_filter` (1805 LOC, ~30 nested early-returns) is decomposed into `fn check_<name>(self, session, ctx) -> Result<Option<Decision>>` helpers (~12 extractions). The top-level becomes a sequence of `if let Some(d) = self.check_X(...)? { return self.write_decision(session, ctx, d).await; }`.
6. The 18 duplicate "build error response + write headers + write body" sites are folded into one `write_decision(session, ctx, decision, extra_headers: &[(&str, String)])` helper (Retry-After + X-RateLimit-Reset on the rate-limit branch becomes a first-class parameter, no more drift).
7. Split lands in 5 commits for `proxy_wiring` and 4 commits for `main.rs` ; each commit keeps `cargo test -p lorica` + `cargo clippy --tests -- -D warnings` green.
8. `cargo expand` diff between before / after every extraction step is empty (zero behaviour change verified mechanically).

### Integration Verification

- IV1: `cargo test --workspace` passes (~2411 tests) at HEAD of the split with byte-identical assertion outputs.
- IV2: `tests-e2e-docker/run.sh` (single-process + workers + cert-export) passes 564 assertions unchanged.
- IV3: The v1.5.2 worker-cert-hot-reload regression test (`proxy_wiring::cert_reload_commit_tests`) still passes ; the deduplicated `BackgroundTasks::start(Role::Worker)` path is exercised by the existing worker-mode Docker suite.
