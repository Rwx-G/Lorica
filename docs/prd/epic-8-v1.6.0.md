...
> five patch releases. Roughly half of the original epic shipped as
...
**Status:** Closed via the v1.5.10 / v1.5.11 patch releases.
...
- v1.5.10: `main.rs` 4329 -> 70 LOC (`cli.rs` + `startup/{supervisor,
  ...
  audit H-9); `proxy_wiring.rs` 5813 -> 2482 LOC (`config / lb /
  context / filters / worker_rpc` submodules); `request_filter`
  decomposed into 22 `check_*` stage methods returning
  `Ok(Some(handled))` / `Ok(None)` (audit H-8); 16/18 terminal error
  responses collapsed onto `write_error_response` with first-class
  `extra_headers` (audit H-10); the `waf_events_clear` rate-limit
  bucket split and the e2e harness repairs also landed there.
- v1.5.11: the two spawn asymmetries this story's original
  extraction had surfaced (workers-mode load-test scheduler never
  started; single-process notification-dispatcher never rebuilt on
  reload) plus the profile-aware e2e teardown.

Two deltas from the legacy implementation were deliberately NOT
taken and are decision points for this cycle:

1. **Retry-After uniformity**: the legacy `Decision` builder put
   `Retry-After` on every rate-limit reject AND the maintenance-mode
   503; main keeps the maintenance 503's historical header order
   byte-for-byte. Decide whether to align when touching that path.
2. **Finer filter split**: legacy split the check helpers into seven
   per-concern files; main keeps one `filters.rs` (~2000 LOC).
...
**Status:** Shipped through the 1.5.x patches: concurrent probing + cold-backend explore/exploit in v1.5.8 (backlog #32, L-18), sharded EWMA tracker in v1.5.10 (backlog #40 L-17). Nothing left in this story.
...
**Status:** Shipped in v1.5.10 (backlog #31 db_blocking + ~100 sites, #30 bot-stash via spawn_blocking, #24 bounded-queue batched log writer + flush barrier). Open delta: the bot-stash still shares the ConfigStore connection (the dedicated synchronous=OFF connection from this story's AC remains a candidate if bot-flood profiling shows the shared WAL as the next bottleneck).
...
**Status:** Partially shipped: the update_settings dedup landed in v1.5.10 (backlog #34, helper functions instead of the SettingsPatch builder, byte-for-byte bounds). Remaining here: wire header_timeout_s + flood_strict_rps (backlog #17) and unify the dual rate-limit paths (backlog #29).
