...
- **Backend lifecycle** - Per-backend active connection tracking via atomic counters. API now exposes `lifecycle_state` (Normal/Closing/Closed) and `active_connections` per backend. Proxy load balancer already filters out Closing/Closed backends from rotation.
