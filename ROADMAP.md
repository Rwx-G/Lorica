...
| 1.4c Bridge tracing spans -> OTel child spans | Planned | 0.5 d | Install `tracing_opentelemetry::OpenTelemetryLayer` in the subscriber so the `#[instrument]` spans on each proxy hook (`request_filter`, `upstream_request_filter`, `response_filter`, `logging`, `fail_to_proxy`) get exported as OTel child spans under the per-request root span built in story 1.4b. Requires solving the provider timing issue (provider installed inside the runtime after settings load, but subscriber is built at startup before the runtime starts); likely solution is to delay subscriber installation to after `otel::init` and move `startup_banner()` from `info!` to `eprintln!` |
