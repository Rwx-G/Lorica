...
pub struct PathRule {
    /// Request-path pattern matched under `match_type`.
    pub path: String,
    /// Prefix / exact-match semantics for `path`.
    #[serde(default)]
    pub match_type: PathMatchType,
    /// If set, swap the route's default backend pool to these IDs
    /// when the rule matches.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub backend_ids: Option<Vec<String>>,
    /// Override `Route.cache_enabled` for matching requests.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub cache_enabled: Option<bool>,
    /// Override `Route.cache_ttl_s` for matching requests.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub cache_ttl_s: Option<i32>,
    /// Replace `Route.response_headers` for matching requests.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub response_headers: Option<HashMap<String, String>>,
    /// Replace `Route.response_headers_remove` for matching requests.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub response_headers_remove: Option<Vec<String>>,
    /// Override the route-level RPS rate limit.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub rate_limit_rps: Option<u32>,
    /// Override the route-level burst token cap.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub rate_limit_burst: Option<u32>,
    /// Short-circuit : redirect matching requests to this URL instead
    /// of proxying. Applied verbatim at the path-rule layer (the
    /// request path / query are NOT appended).
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub redirect_to: Option<String>,
    /// Short-circuit : terminate matching requests with this HTTP
    /// status instead of proxying. Combines with `error_page_html`.
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
pub struct HeaderRule {
    ...
    /// compared against the header's value.
    pub value: String,
    /// Backends serving the matched request. Empty = match but
    /// fall through to the route's default backend selection.
    #[serde(default)]
...
pub struct ResponseRewriteRule {
    /// Literal string or regex (see `is_regex`) searched against the
    /// buffered response body.
    pub pattern: String,
    /// Replacement string. For regex rules `$N` references capture
    /// groups (1-indexed).
    pub replacement: String,
    /// Whether `pattern` is compiled as a regex (default literal).
    #[serde(default)]
    pub is_regex: bool,
    /// Cap on the number of matches replaced per response. `None`
    /// = unlimited. Protects against pathological blow-up rules.
    #[serde(default)]
...
/// mTLS client-certificate verification for a single route. See the
/// preceding module-level comment for the threat model.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Route {
    /// Stable UUID; primary key for `ConfigStore::get_route` and the
    /// `route_backends` join table. Generated by the API on create.
    pub id: String,
    /// Primary hostname the route answers on (lowercase DNS name).
    /// Unique across the route table.
    pub hostname: String,
    /// URL path prefix the route answers under (e.g. `"/api"`).
    /// Requests to `https://{hostname}{path_prefix}...` land here.
    pub path_prefix: String,
    /// ID of the [`Certificate`] used to terminate TLS for this
    /// route. `None` means plaintext-only (HTTP listener).
    ///
    /// [`Certificate`]: super::certificate::Certificate
    pub certificate_id: Option<String>,
    /// Load-balancing policy across `route_backends`.
    pub load_balancing: LoadBalancing,
    /// Enables the WAF filter chain for this route.
    pub waf_enabled: bool,
    /// `Monitor` (log only) vs `Block` (reject 403) when a WAF rule
    /// fires.
    pub waf_mode: WafMode,
    /// Admin toggle. Disabled routes keep their config but are
    /// skipped by the router (404 for traffic hitting the host).
    pub enabled: bool,
    /// Force redirect to HTTPS on the plaintext port.
    #[serde(default)]
    pub force_https: bool,
    /// Redirect host for route-level redirects (schema-less DNS
    /// name, validated via `validate_redirect_hostname`).
    #[serde(default)]
    pub redirect_hostname: Option<String>,
    /// Redirect target URL at route level. When set, every request
    /// on this route is rewritten to `{redirect_to}{path}{query}`.
    #[serde(default)]
    pub redirect_to: Option<String>,
    /// Extra hostnames the route answers on (DNS CNAME pattern).
    /// Must be unique across the route table, same rule as
    /// `hostname`. Wildcard entries (`*.example.com`) allowed.
    #[serde(default)]
    pub hostname_aliases: Vec<String>,
    /// Request headers added on the way to the upstream.
    #[serde(default)]
    pub proxy_headers: HashMap<String, String>,
    /// Response headers appended on the way back to the client.
    #[serde(default)]
    pub response_headers: HashMap<String, String>,
    /// Named security-header preset (`"strict"`, `"moderate"`,
    /// `"relaxed"`, `"none"`). Hardening layer on top of
    /// `response_headers`.
    ...
    /// Upstream send timeout in seconds.
    #[serde(default = "default_send_timeout_s")]
    pub send_timeout_s: i32,
    /// If set, strip this prefix from the request path before
    /// forwarding (e.g. `"/api"` turns `/api/foo` into `/foo`).
    #[serde(default)]
    pub strip_path_prefix: Option<String>,
    /// If set, prepend this prefix to the request path before
    /// forwarding. Applied after `strip_path_prefix`.
    #[serde(default)]
    ...
    /// Whether the proxy emits an access log line per request on
    /// this route.
    #[serde(default = "default_access_log_enabled")]
    pub access_log_enabled: bool,
    /// Request headers removed before forwarding to the upstream.
    #[serde(default)]
    pub proxy_headers_remove: Vec<String>,
    /// Response headers removed before returning to the client.
    #[serde(default)]
    ...
    /// Per-client RPS rate-limit (see also the richer `rate_limit`
    /// struct below). `None` = unlimited.
    #[serde(default)]
    pub rate_limit_rps: Option<u32>,
    /// Burst / token-bucket capacity paired with `rate_limit_rps`.
    #[serde(default)]
    pub rate_limit_burst: Option<u32>,
    /// Per-route IP allow-list (CIDRs). Non-empty list rejects any
    /// client IP not in it.
    #[serde(default)]
    pub ip_allowlist: Vec<String>,
    /// Per-route IP deny-list (CIDRs). Rejects matching clients
    /// with 403.
    #[serde(default)]
    pub ip_denylist: Vec<String>,
    /// CORS `Access-Control-Allow-Origin` values (`*`, `null`, or
    /// explicit origins).
    #[serde(default)]
    pub cors_allowed_origins: Vec<String>,
    /// CORS `Access-Control-Allow-Methods` values (uppercase HTTP
    /// verbs).
    #[serde(default)]
    pub cors_allowed_methods: Vec<String>,
    /// CORS preflight `Access-Control-Max-Age` in seconds.
    #[serde(default)]
    pub cors_max_age_s: Option<i32>,
    /// Enables gzip / deflate response compression.
    #[serde(default = "default_compression_enabled")]
    pub compression_enabled: bool,
    /// Number of upstream retry attempts on idempotent methods.
    #[serde(default)]
    pub retry_attempts: Option<u32>,
    /// Enables the in-proxy response cache on this route.
    #[serde(default)]
    pub cache_enabled: bool,
    /// Default cache entry TTL in seconds when the origin does not
    /// set one.
    #[serde(default = "default_cache_ttl_s")]
    pub cache_ttl_s: i32,
    /// Cap on cached response body size in bytes. Responses larger
    /// than this bypass the cache and stream through.
    #[serde(default = "default_cache_max_bytes")]
    pub cache_max_bytes: i64,
    /// Hard cap on concurrent connections on this route. `None` =
    /// unlimited.
    #[serde(default)]
    ...
    /// auto-banner blocks the client IP. `None` = auto-ban off.
    #[serde(default)]
    pub auto_ban_threshold: Option<u32>,
    /// Ban duration in seconds applied by the auto-banner.
    #[serde(default = "default_auto_ban_duration_s")]
    pub auto_ban_duration_s: i32,
    /// Per-path overrides evaluated in declaration order after
    /// `header_rules` and `traffic_splits`.
    #[serde(default)]
    pub path_rules: Vec<PathRule>,
    /// Route-level short-circuit : respond with this HTTP status
    /// instead of proxying. Combine with `error_page_html`.
    #[serde(default)]
    pub return_status: Option<u16>,
    ...
    /// Username for the optional HTTP Basic auth gate. `None` =
    /// disabled.
    #[serde(default)]
    pub basic_auth_username: Option<String>,
    /// Argon2id hash of the Basic-auth password. Compared with
    /// constant-time verify on each request.
    #[serde(default)]
    ...
    /// Cache-Control `stale-if-error` window in seconds.
    #[serde(default = "default_stale_if_error_s")]
    pub stale_if_error_s: i32,
    /// HTTP methods the retry logic is allowed to re-issue. Defaults
    /// to `GET` / `HEAD` / `OPTIONS` when empty.
    #[serde(default)]
    pub retry_on_methods: Vec<String>,
    /// When true, the route returns 503 on every request without
    /// hitting the upstream. Combine with `error_page_html` for a
    /// custom maintenance page.
    #[serde(default)]
    pub maintenance_mode: bool,
    /// Operator-supplied HTML served for terminal status codes on
    /// this route. Sanitised through `ammonia` before rendering.
    #[serde(default)]
...
pub struct GeoIpConfig {
    /// Allowlist vs denylist semantics for `countries`.
    pub mode: GeoIpMode,
...
pub enum BotProtectionMode {
    /// Passive verdict-cookie refresh (no interactive step).
    Cookie,
    /// JavaScript proof-of-work run in the browser.
    Javascript,
    /// Image captcha typed by the user.
    Captcha,
}
...
pub struct BotProtectionConfig {
    /// Which challenge mode the proxy issues when the bypass matrix
    /// does not match.
