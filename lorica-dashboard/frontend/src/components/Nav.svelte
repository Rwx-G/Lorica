<script lang="ts">
  ...
  // `operator: true` hides the entry from a Viewer: every read on that
  // page sits at the Operator floor, so a Viewer would open a page of
  // 403s.
  type NavItem =
    | { path: string; label: string; icon: string; cluster?: boolean; operator?: boolean }
  ...
  const navItems: NavItem[] = [
    ...
    { path: '/capture', label: 'Capture', icon: 'aperture', operator: true },
  ...
  const visibleItems = $derived(
    navItems.filter(
      (i) =>
        (!i.cluster || (clustered && $canWriteRole)) &&
        (!('operator' in i) || !i.operator || $canWriteRole),
    ),
  );
