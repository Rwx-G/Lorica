...
use bytes::BufMut;
use http::Version;
use lorica_error::{Error, ErrorType, ImmutStr, Result};
...
// Should match lorica http1 setting
...
fn buf_to_http_header(buf: &[u8]) -> Result<ResponseHeader> {
    ...
    // `lorica-core/src/protocols/http/v1/client.rs`): allow spaces between
...
fn parsed_to_header(parsed: &httparse::Response) -> Result<ResponseHeader> {
    ...
    for header in parsed.headers.iter() {
        let header_value = lorica_http::header_value_from_slice(header.value);
...
mod tests {
    ...
    fn test_ser_wo_dict() {
        ...
        header.append_header("Server", "Lorica").unwrap();
