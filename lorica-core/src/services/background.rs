...
//!
//! A [BackgroundService] can be run as part of a Lorica application to add supporting logic that
...
/// but you shouldn't implement both. Under the hood, the lorica service will
/// call the `start_with_ready_notifier` function. By default this function will
/// call the regular `start` function.
#[async_trait]
pub trait BackgroundService {
    /// This function is called when the lorica server tries to start all the
...
impl<A> GenBackgroundService<A> {
    /// Generates a background service that can run in the lorica runtime
