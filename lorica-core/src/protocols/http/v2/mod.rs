...
mod test {
    ...
    async fn test_client_write_timeout() {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let err = h2_stream
                ...
                .err()
                .unwrap();
            assert_eq!(err.etype(), &lorica_error::ErrorType::WriteTimedout);
        }));
    ...
    async fn test_graceful_shutdown_processes_inflight_stream() {
        ...
        let client_handle = tokio::spawn(async move {
            ...
            let _ = lorica_timeout::timeout(Duration::from_secs(5), async {
    ...
    async fn test_graceful_shutdown_idle_connection_exits_promptly() {
        ...
        let client_handle = tokio::spawn(async move {
            ...
            let mut saw_goaway = false;
            let _ = lorica_timeout::timeout(Duration::from_secs(3), async {
                while let Some(frame) = codec.next().await {
                    if matches!(frame.unwrap(), h2::frame::Frame::GoAway(_)) {
                        saw_goaway = true;
                        break;
        ...
        let result = lorica_timeout::timeout(
    ...
    async fn test_h2_idle_timeout_starts_after_active_stream_finishes() {
        ...
        let active_session = lorica_timeout::timeout(Duration::from_secs(1), guard_rx)
            .await
        ...
        lorica_timeout::timeout(Duration::from_secs(1), accept_handle)
            .await
