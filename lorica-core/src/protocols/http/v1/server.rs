...
use lorica_http::{IntoCaseHeaderName, RequestHeader, ResponseHeader};
...
impl HttpSession {
    ...
    pub async fn reuse(mut self) -> Result<Option<Stream>> {
        if !self.will_keepalive() {
            ...
            // pipelining support, and lorica 400s if pipelined requests are sent
...
mod tests_stream {
    ...
    async fn read_2_buf() {
        ...
        assert_eq!(b"lorica.org", http_stream.get_header_bytes("Host"));
    }
    ...
    async fn transfer_encoding_and_content_length_disallowed(
        ...
        init_log();
        let input1 = b"GET / HTTP/1.1\r\n";
        let mut input2 = "Host: lorica.org\r\n".to_owned();
...
mod test_timeouts {
    ...
    fn mocked_blocking_body_forever_stream() -> Box<Mock> {
        ...
        let http2 = b"Host: lorica.example\r\nContent-Length: 3\r\n\r\n";
        Box::new(
            Builder::new()
                .read(&http1[..])
                .read(&http2[..])
    ...
    async fn test_read_http_request_headers_timeout_for_read_request() {
        ...
        assert!(res.is_err()); // test timeout occurred, and not any internal Lorica timeout
