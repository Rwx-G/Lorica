...
use log::{debug, trace};
use lorica_error::{Error, ErrorType::*, OrErr, Result, RetryType};
use lorica_http::{HMap, IntoCaseHeaderName, RequestHeader, ResponseHeader};
...
impl HttpSession {
    ...
    fn fail_response_header_read<T>(&mut self, e: lorica_error::BError) -> Result<T> {
                ...
                match parsed {
                    HeaderParseState::Complete(s) => {
                        ...
                        for header in header_refs {
                            ...
                            let header_value = lorica_http::header_value_from_raw(value_bytes);
    ...
    pub async fn reuse(mut self) -> Option<Stream> {
        // A body that stopped short of its framing-defined end, or one
        // that left bytes in the read buffer, means this connection is
        // not at a message boundary. Putting it back in the pool hands
        // the next request a stream that starts mid-garbage, which is a
        // cross-request desync (upstream 1ed40d42).
...
mod tests_stream {
    ...
    // via `lorica_http::header_value_from_raw`.
    #[tokio::test]
