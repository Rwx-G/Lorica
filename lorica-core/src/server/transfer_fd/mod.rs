...
mod tests {
    ...
    fn test_send_receive_fds() {
        ...
        let child = thread::spawn(move || {
            ...
            let (fds, bytes) =
                get_fds_from("/tmp/lorica_fds_receive.sock", &mut buf, None).unwrap();
            debug!("{:?}", fds);
            assert_eq!(1, fds.len());
            assert_eq!(32, bytes);
            assert_eq!(1, buf[0]);
        ...
        match send_fds_to(fds, &buf, "/tmp/lorica_fds_receive.sock", None) {
            Ok(sent) => {
                assert!(sent > 0);
            }
            Err(e) => {
    ...
    fn test_serde_via_socket() {
        ...
        fds.send_to_sock("/tmp/lorica_fds_receive2.sock").unwrap();
        child.join().unwrap();
    }
    ...
    fn test_send_fds_to_respects_configurable_timeout() {
        ...
        let result = send_fds_to(fds, &buf, "/tmp/lorica_test_config_send.sock", Some(2));
    ...
    fn test_get_fds_from_respects_configurable_timeout() {
        ...
        let start = Instant::now();
        let result = get_fds_from("/tmp/lorica_test_config_receive.sock", &mut buf, Some(2));
        let elapsed = start.elapsed();
